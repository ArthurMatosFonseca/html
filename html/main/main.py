...
def criar_lembrete(horario, mensagem):
    horario = normalizar_horario(horario)
    if horario is None:
        return None

    mensagem = mensagem.strip() or "Lembrete programado."
    ...
    try:
        ...
        cursor = conexao.cursor()
        cursor.execute("""
            INSERT INTO lembretes (horario, mensagem, disparado, criado_em)
            VALUES (?, ?, 0, ?)
...
def listar_lembretes():
    ...
    try:
        cursor = conexao.cursor()
        cursor.execute("""
            SELECT id, horario, mensagem, disparado, criado_em
            FROM lembretes
            WHERE disparado = 0
            ORDER BY horario, id
        """)
...
def apagar_lembrete(id_lembrete):
    conexao = conectar_banco()
    try:
        cursor = conexao.cursor()
...
def verificar_lembretes():
    """Verifica os lembretes a cada segundo e avisa quando chegar a hora."""
    while True:
        ...
        try:
            ...
            cursor.execute("""
                ...
                FROM lembretes
                WHERE disparado = 0 AND horario <= ?
                ORDER BY horario, id
            """, (agora,))
            lembretes = cursor.fetchall()

            for id_lembrete, horario, mensagem in lembretes:
                cursor.execute("""
                    UPDATE lembretes
                    SET disparado = 1
                    WHERE id = ? AND disparado = 0
                """, (id_lembrete,))
                ...
                if cursor.rowcount:
                    with PRINT_LOCK:
                        print("\a")
                        print("\n" + "=" * 60)
                        print("        [JARVIS ALERTA DE HORÁRIO]")
                        print("=" * 60)
...
def iniciar_verificador_lembretes():
    thread_lembretes = threading.Thread(
        target=verificar_lembretes,
        daemon=True
    )
    thread_lembretes.start()


def interpretar_lembrete(texto):
    """
    Exemplos:
      me avise às 09:30
      me avise às 09:30 para estudar
      lembre-me às 14:00 de fazer a tarefa
      lembrete 18:45 reunião
      avise-me 20:10
    """
    texto_limpo = texto.strip()

    padrao = re.search(
        r"\b(?:às|as)\s+(\d{1,2}:\d{2}(?::\d{2})?)\b(.*)$",
        texto_limpo,
        re.IGNORECASE
    )
    ...
    if not padrao:
        padrao = re.search(
            r"\b(?:avise(?:-me)?|avisa(?:-me)?|lembre(?:-me)?|lembrete)"
            r"(?:\s+me)?\s+(\d{1,2}:\d{2}(?::\d{2})?)\b(.*)$",
            texto_limpo,
            re.IGNORECASE
        )
    ...
    horario = normalizar_horario(padrao.group(1))
    if horario is None:
        return "Horário inválido.\nUse o formato HH:MM, por exemplo: 09:30."

    complemento = padrao.group(2).strip()
    complemento = re.sub(
        r"^(?:para|de|que|me|a|o)\s+",
        "",
        complemento,
        flags=re.IGNORECASE
    ).strip()

    mensagem = complemento or "Seu horário programado chegou."
    id_lembrete = criar_lembrete(horario, mensagem)
    ...
    return (
        ...
        f"Mensagem: {mensagem}\n"
        f"ID: {id_lembrete}\n\n"
        "Ficarei verificando o horário e avisarei quando chegar."
    )
...
def normalizar_equacao_quadratica(texto):
    ...
    comandos = [
        r"\bresolva\b", r"\bresolver\b", r"\bcalcule\b", r"\bcalcular\b",
        r"\bcalcula\b", r"\bencontre\b", r"\bencontrar\b", r"\bdetermine\b",
        r"\bdeterminar\b", r"\bach[eé]\b", r"\bachar\b", r"\bobtenha\b",
        r"\bobter\b", r"\bdescubra\b", r"\bdescobrir\b", r"\braízes?\s+de\b",
        r"\braizes?\s+de\b", r"\bequação\b", r"\bequacao\b", r"\bequações\b",
        r"\bequacoes\b", r"\bsegundo\s+grau\b", r"\b2º\s+grau\b", r"\b2o\s+grau\b",
        r"\b2\s+grau\b", r"\bquadrática\b", r"\bquadratica\b", r"\bbhaskara\b",
        r"\bbaskara\b"
    ]
...
def interpretar_pa(texto):
    ...
    r = extrair_parametro(r"\br\s*=?\s*(-?\d+(?:[.,]\d+)?)", texto_l, "r")
    n = extrair_parametro(r"\bn\s*=?\s*(\d+)", texto_l, "n")
...
def interpretar_pg(texto):
    texto_l = texto.lower()
    a1 = extrair_parametro(r"a1\s*=?\s*(-?\d+(?:[.,]\d+)?)", texto_l, "a1")
    q = extrair_parametro(r"\bq\s*=?\s*(-?\d+(?:[.,]\d+)?)", texto_l, "q")
    n = extrair_parametro(r"\bn\s*=?\s*(\d+)", texto_l, "n")
...
def interpretar_matematica(texto):
    ...
    # PA / PG
    if re.search(r"\bpa\b", texto_lower) or "progressão aritmética" in texto_lower or "progressao aritmetica" in texto_lower:
        return interpretar_pa(texto)

    if re.search(r"\bpg\b", texto_lower) or "progressão geométrica" in texto_lower or "progressao geometrica" in texto_lower:
...
def responder(mensagem):
    ...
    # LEMBRETES
    resultado_lembrete = interpretar_lembrete(mensagem)
    if resultado_lembrete:
        return resultado_lembrete

    if texto in ["lembretes", "listar lembretes", "meus lembretes"]:
        lembretes = listar_lembretes()
        if not lembretes:
            return "Não há lembretes pendentes."

        resposta = "LEMBRETES PENDENTES\\n-------------------------\\n"
        for id_l, horario, msg, _, criado in lembretes:
            resposta += f"{id_l}. {horario} → {msg}\\n"
        return resposta

    match_apagar = re.fullmatch(r"(?:apagar|cancelar)\s+lembrete\s+(\d+)", texto)
    if match_apagar:
        id_l = int(match_apagar.group(1))
        if apagar_lembrete(id_l):
            return f"Lembrete {id_l} cancelado."
        return f"Lembrete {id_l} não encontrado."
